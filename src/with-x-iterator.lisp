  ...
  (labels ((rec (binds &optional generators)
             (if (endp binds)
                 ...
                 (let ((type (form-type (cadar binds) env))
                   ...
                   `((,(<with-x-iterator> type) (,?generate ,(cadar binds))
                      ,@(rec (cdr binds) (cons ?generate generators)))))))
           (<with-x-iterator> (type)
             (cond ((subtypep type 'list) 'with-list-iterator)
                   ((subtypep type 'vector) 'with-vector-iterator)
                   ((subtypep type 'sequence) 'with-sequence-iterator)
                   ((subtypep type 'hash-table) 'with-hash-table-iterator)
                   ((subtypep type 'integer) 'with-integer-iterator)
